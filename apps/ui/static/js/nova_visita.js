...
// Fun+�+�o para verificar se estamos na p+�gina de gerenciamento
...
function mostrarBuscaVeiculo() {
  ...
  if (typeof modalBuscaVeiculo !== 'undefined' && modalBuscaVeiculo) {
    ...
    console.error('Modal Bootstrap de busca de ve+�culo n+�o inicializado');
  }
}
...
// Adicionar pessoa +� tabela (apenas para nova visita)
function adicionarPessoaATabela(pessoa) {
  if (isModoGerenciamento()) {
    // No modo gerenciamento, adicionar diretamente +� visita existente
  ...
  if (pessoasAdicionadas.some(p => p.id === pessoa.id)) {
    alert("Esta pessoa j+� foi adicionada +� visita");
    return;
  }
...
// Adicionar ve+�culo +� tabela (apenas para nova visita)
...
async function cancelarVisita() {
  ...
  const confirmar = await (window.showConfirm ? window.showConfirm('Deseja realmente cancelar esta visita? Todos os dados ser�o perdidos.', 'Cancelar visita') : Promise.resolve(window.confirm('Deseja realmente cancelar esta visita? Todos os dados ser�o perdidos.')));
  if (!confirmar) return;
  ...
  // if (confirm("Deseja realmente cancelar esta visita? Todos os dados ser+�o perdidos.")) {
    ...
    // Limpar valores dos inputs b�sicos
...
// Adicionar ve+�culo +� visita existente (para modo gerenciamento)
...
document.addEventListener("DOMContentLoaded", () => {
  // Atualizar vari+�veis globais se estiver em modo gerenciamento
  ...
  // Salvar rascunho ao editar campos b�sicos
  ...
  if (!isModoGerenciamento()) {
    ...
    if (btnNovaPessoa) {
      btnNovaPessoa.addEventListener("click", () => {
        alert("Funcionalidade de cadastrar nova pessoa ser+� implementada aqui");
      });
    }
    ...
    if (btnNovoVeiculo) {
      btnNovoVeiculo.addEventListener("click", () => {
        alert("Funcionalidade de cadastrar novo ve+�culo ser+� implementada aqui");
      });
    }
...
// EXPORTAR FUN+�+�ES GLOBAIS PARA USO EM OUTROS ARQUIVOS
